...
## Donderdag 19-3-2026
Ik heb in de standup aangegeven dat ik de laatste dingen aan de style dictionary ging doen. Ik heb het in de ochtend met Tessa gehad over hoe de stage bevalt en of ik het nog een beetje zie zitten. Aan haar ook verteld dat ik alles mega interessant vind maar het allemaal wel heel anders is als bij fdnd, en dat ik erg moet wennen aan dit vlak van het frontend vak. Tessa gaf aan dat het bij haar in het begin zeker ook zo was en dat ik altijd iedereen kan vragen om hulp of steun. Ze gaf me ook een aantal onboarding tips en bronnen waar ik even naar kon kijken:
- [Onboarding Frameless](https://docs.google.com/document/d/1VZ1LTCQgQbb3WL2-wIRnU-oNDX-Xzv3QIEi7qeIBzMI/edit?tab=t.0#heading=h.l11v5fqcbrqy) in google docs
- [Onboarding sessie voor NL Design System](https://youtu.be/Yh4NqpphJQ4?si=68_5ZDlrj311fqoZ) filmpje 
- [Onboarding werken met NL Design System](https://github.com/nl-design-system/example/blob/main/README.md) Readme van de example repo
- [Handboek NL Design System](https://www.nldesignsystem.nl/handboek/) op de NL Design System website
Na de lunch heb ik met Robbert naar de error in mijn pull request gekeken. Hij zag meteen waar het mis ging, ik had op twee plekken teveel de `tokens.json` aangepast naar het `fdnd.tokens.json` bestand. Ik had dit in het `basis-design-tokens` mapje `style-dictionary.config.mjs` aangepast wat niet de bedoeling was en in het `style-dicionary.config.js` op het hoogste niveau hetzelfde. Ik had dit alleen in het `ma-design-tokens` mapje hoeven doen. Ik was dus te moeilijk aan het nadenken. Dit had ik terug gedraaid met `git checkout` specifiek voor het eerste bestand: `git checkout origin/main ../basis-design-tokens/style-dictionary.config.mjs`. En kreeg hierna gelijk een git lesje over wat er allemaal mogenlijk is. Daarna hebben we samen naar het onboarding google docs bestand gekeken en een aantal handige extenties toegevoegd aan mijn vs code. <br>
Vanuit mijn [pull request](https://github.com/nl-design-system/themes/pull/1379) kan ik nu ook de [storybook preview](https://themes-git-anouk-nl-design-system.vercel.app/?path=/docs/mooi-anders-color--docs) laten zien. We gaan morgen verder naar de chromatics kijken. Voor nu ga ik proberen of ik het in mijn stage repo ook aan de praat kan krijgen. Ik heb nog een [articles.md](articles.md) bestand aangemaakt waar ik de gelezen bronnen in bij kan houden zodat ik daar een duidelijk overzicht van heb.

## Vrijdag 20-3-2026
In de standup heb ik uitgelegd dat ik afgelopen week bezig ben geweest om het fdnd thema in de themes repo van de NL Design System github te krijgen en dat dat nu gelukt is en dat ik vandaag wil proberen om dat ook in mijn stage repo van frameless te krijgen. <br>
Daar ben ik vanochtend mee verder gegaan en ik was weer bij de meeting met het kernteam van utrecht. Toen kreeg ik een side quest van Robbert om de utrecht repo van de nl-design-system github te clonen. Dit heb ik via de git command lines gedaan:
- git clone SSH url uit github
- met cd naar de map genavigeerd door in de finder de map te vinden het het path te kopieren
- pnpm install gedaan
- pnpm run build
- en pnpm run storybook om het storybook op te starten
Na de lunch kwam Dorien langs op stagebezoek. We hebben even gekletst over hoe het gaat op stage en hoe ik het vind. Ik zei dat ik het tot nu toe erg leuk vind maar dat het wel erg veel nieuwe informatie is en ik nog aan het wennen ben aan de nieuwe manier van werken. Robbert zei dat het hartstikke goed gaat omdat ik erg leergierig ben, zijn flauwe grapjes kan waarderen en ze ook terug maak en dat ik goed dingen zelf kan uitzoeken. Ook vind hij dat ik het werken met een mac en de git command dingen snel oppak. Het was fijn om dat te horen! Met Dorien en Robbert zijn we samen nog even door de indicatoren gelopen om te kijken wat voorbewijslast ik al heb en wat ik nog moet verzamelen. Ik gaf aan dat ik nu twee keer de onderzoeks fase heb gehad en daardoor niet zo heel veel complexe frontend code heb geschreven. Robbert gaf aan dat ik daar komende week nog wel mee aan de slag kon gaan. <br>
Ik ben verder gegaan met het utrecht thema om een component te dupliceren naar van het css storybook naar het react storybook. Ik heb het bestand en de bijbehorende map gekopieerd uit de css map en aan de react map toegevoegd. Ondertussen werdt ik door Robbert een meeting in geroepen omdat hij iets leuks wilde laten zien. Hij vroeg aan mij waar ik de css selectors altijd op zet, de root of de body, ik zei de body en vertelde waarom. Hij liet zien dat je dus ook als je je css op de root zet dit op de website kan laten zien door je <head> display block mee te geven Daarna heb ik samen met Robbert de errors aangepast die door de gedupliceerde namen kwamen. 